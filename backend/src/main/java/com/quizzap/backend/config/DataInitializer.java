...
public class DataInitializer implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    logger.info("=== DATA INITIALIZER START ===");

    // ===== CATEGORIES =====
    logger.info("Categories count: {}", categoryRepository.count());
    if (categoryRepository.count() == 0) {
      logger.info("Inserting categories...");
      Category videogames = categoryRepository.save(
          new Category(null, "videogames", "If you love video games, this is the right category for you",
              "sports_esports"));
      Category cinema = categoryRepository.save(
          new Category(null, "cinema", "Find out how much you know about movies and actors", "movie"));
      Category geography = categoryRepository.save(
          new Category(null, "geography", "Explore the world by answering geography questions", "public"));
      Category history = categoryRepository.save(
          new Category(null, "history", "Travel through time with questions about world history", "history_edu"));
      Category art = categoryRepository.save(
          new Category(null, "art", "Test your knowledge about painting and artistic movements", "palette"));
      Category science = categoryRepository.save(
          new Category(null, "science", "Challenge your scientific knowledge in various fields", "science"));
      logger.info("Categories inserted: {}", categoryRepository.count());
    } else {
      logger.info("Categories already exist, skipping...");

    }

    logger.info("Questions count: {}", questionRepository.count());
    if (questionRepository.count() == 0) {
      logger.info("Inserting questions...");
      try {

        Category videogames = categoryRepository.findByName("videogames").orElseThrow();
        Category cinema = categoryRepository.findByName("cinema").orElseThrow();
        Category geography = categoryRepository.findByName("geography").orElseThrow();
        Category history = categoryRepository.findByName("history").orElseThrow();
        Category art = categoryRepository.findByName("art").orElseThrow();
        ...
        questionRepository.save(new Question(null, "Which game studio developed The Elder Scrolls V: Skyrim?",
            Arrays.asList("CD Projekt Red", "Bethesda", "BioWare", "Ubisoft"),
            1, videogames));
        questionRepository.save(new Question(null, "In Minecraft, what is the Nether?",
            Arrays.asList("A type of ore", "A hostile dimension", "A crafting station", "A rare mob"),
            1, videogames));
        questionRepository.save(new Question(null, "Which fighting game features characters like Ryu and Ken?",
            Arrays.asList("Tekken", "Mortal Kombat", "Street Fighter", "Dead or Alive"),
            2, videogames));
        questionRepository.save(new Question(null, "What is the name of the virus in the Last of Us?",
            Arrays.asList("Cordyceps", "Z-Virus", "T-Virus", "Fungus"),
            0, videogames));
        questionRepository.save(new Question(null, "Which game features a character named Geralt of Rivia?",
            Arrays.asList("Dragon Age", "The Witcher", "Dark Souls", "Elder Scrolls"),
            1, videogames));
        questionRepository
            .save(new Question(null, "What is the main protagonist's name in the Assassin's Creed series?",
                Arrays.asList("Ezio", "Altaïr", "Desmond", "There are multiple protagonists"),
                3, videogames));
        questionRepository.save(new Question(null, "Which of these is NOT a real video game genre?",
            Arrays.asList("Souls-like", "Metroidvania", "Rogue-lite", "Platformer-shooter"),
            3, videogames));
        questionRepository
            .save(new Question(null, "What is the name of the princess you must rescue in the Super Mario series?",
                Arrays.asList("Daisy", "Toad", "Zelda", "Peach"),
                3, videogames));
        ...
        questionRepository.save(new Question(null, "Which actor played Jack in Titanic?",
            Arrays.asList("Leonardo DiCaprio", "Brad Pitt", "Johnny Depp", "Tom Cruise"),
            0, cinema));
        questionRepository.save(new Question(null, "Which film won Best Picture at the 2020 Oscars?",
            Arrays.asList("1917", "Joker", "Parasite", "The Irishman"),
            2, cinema));
        questionRepository
            .save(new Question(null, "Who played the role of Tony Stark in the Marvel Cinematic Universe?",
        ...
        questionRepository.save(new Question(null, "Mount Everest is located in which mountain range?",
            Arrays.asList("Andes", "Alps", "Himalayas", "Rockies"),
            2, geography));
        questionRepository.save(new Question(null, "Which country has the most natural lakes?",
            Arrays.asList("Russia", "Canada", "Kenya", "Finland"),
            1, geography));
        ...
        questionRepository.save(new Question(null, "Which continent contains the most countries?",
            Arrays.asList("Asia", "Africa", "Europe", "South America"),
            1, geography));
        ...
        questionRepository.save(new Question(null, "Which country is shaped like a boot?",
            Arrays.asList("Greece", "Spain", "Portugal", "Italy"),
            3, geography));
        ...
        questionRepository.save(new Question(null, "Which art movement is characterized by small dots of color?",
            Arrays.asList("Impressionism", "Pointillism", "Fauvism", "Expressionism"),
            1, art));
        ...
        questionRepository.save(new Question(null,
            ...
            Arrays.asList("Leonardo da Vinci", "Filippo Brunelleschi", "Sandro Botticelli", "Michelangelo"),
            1, art));
        questionRepository.save(new Question(null, "Which artist cut off part of his own ear?",
            Arrays.asList("Pablo Picasso", "Vincent van Gogh", "Salvador Dalí", "Edvard Munch"),
            1, art));
        questionRepository.save(
            new Question(null,
                "What is the name of the famous painting by Edvard Munch that depicts a figure screaming?",
                Arrays.asList("The Scream", "The Shout", "The Cry", "The Yell"),
                0, art));
        questionRepository
            .save(new Question(null, "Which art movement was founded by Pablo Picasso and Georges Braque?",
                Arrays.asList("Surrealism", "Cubism", "Dadaism", "Futurism"),
                1, art));
        questionRepository.save(new Question(null, "What is the main medium used in watercolor painting?",
            Arrays.asList("Oil", "Acrylic", "Water-soluble pigments", "Charcoal"),
            2, art));
        questionRepository.save(new Question(null, "Which famous art museum is located in Madrid?",
            Arrays.asList("The Louvre", "The Prado", "The Tate", "The Uffizi"),
            1, art));
        questionRepository.save(new Question(null, "Who created the sculpture The Thinker?",
            Arrays.asList("Michelangelo", "Auguste Rodin", "Donatello", "Alberto Giacometti"),
            1, art));
        questionRepository.save(new Question(null, "Which art movement is characterized by dream-like scenes?",
            Arrays.asList("Impressionism", "Surrealism", "Realism", "Baroque"),
            1, art));
        questionRepository.save(new Question(null,
            "What is the name of the famous painting by Grant Wood featuring a farmer and his daughter?",
            Arrays.asList("American Gothic", "The Farmers", "Rural Life", "Midwest Portrait"),
            0, art));
        questionRepository.save(new Question(null, "Which artist is known for his drip paintings?",
            Arrays.asList("Jackson Pollock", "Willem de Kooning", "Mark Rothko", "Andy Warhol"),
            0, art));
        questionRepository
            .save(
                new Question(null, "What is the name of the famous ancient Greek statue depicting the goddess of love?",
                    Arrays.asList("Venus de Milo", "Winged Victory", "Discobolus", "Laocoön"),
                    0, art));

        // HISTORY
        questionRepository.save(new Question(null, "In which year did World War II end?",
            Arrays.asList("1945", "1939", "1950", "1941"),
            0, history));
        questionRepository.save(new Question(null, "Who was the first emperor of Rome?",
            Arrays.asList("Julius Caesar", "Nero", "Augustus", "Tiberius"),
            2, history));
        questionRepository.save(new Question(null, "The Renaissance began in which country?",
            Arrays.asList("France", "Germany", "Italy", "England"),
            2, history));
        ...
        questionRepository.save(new Question(null, "What force keeps planets in orbit around the sun?",
            Arrays.asList("Electromagnetism", "Gravity", "Nuclear Force", "Centrifugal Force"),
            1, science));
        questionRepository.save(new Question(null, "What is the speed of light?",
            Arrays.asList("300,000 km/s", "150,000 km/s", "1,000,000 km/s", "500,000 km/s"),
            0, science));
        ...
        questionRepository.save(new Question(null, "What is the boiling point of water?",
            Arrays.asList("100°C", "0°C", "50°C", "25°C"),
            0, science));
        ...
        questionRepository.save(new Question(null, "What is the chemical symbol for Gold?",
            Arrays.asList("Au", "Ag", "Pb", "Fe"),
            0, science));
        questionRepository.save(new Question(null, "What is the powerhouse of the cell?",
            Arrays.asList("Nucleus", "Mitochondria", "Ribosome", "Chloroplast"),
            1, science));
        logger.info("Questions inserted: {}", questionRepository.count());
      } catch (Exception e) {
        logger.error("ERROR inserting questions: ", e);
      }
    } else {
    ...
    if (userRepository.count() == 0) {
      ...
      try {
        User mario = userRepository.save(new User(null, "mario", "mario@example.com", passwordEncoder.encode("mario")));
        User luigi = userRepository.save(new User(null, "luigi", "luigi@example.com", passwordEncoder.encode("luigi")));
        User peach = userRepository.save(new User(null, "peach", "peach@example.com", passwordEncoder.encode("peach")));
        User bowser = userRepository
            .save(new User(null, "bowser", "bowser@example.com", passwordEncoder.encode("bowser")));
        User yoshi = userRepository.save(new User(null, "yoshi", "yoshi@example.com", passwordEncoder.encode("yoshi")));
        User toad = userRepository.save(new User(null, "toad", "toad@example.com", passwordEncoder.encode("toad")));
        User wario = userRepository.save(new User(null, "wario", "wario@example.com", passwordEncoder.encode("wario")));
        User waluigi = userRepository
            .save(new User(null, "waluigi", "waluigi@example.com", passwordEncoder.encode("waluigi")));
        User daisy = userRepository.save(new User(null, "daisy", "daisy@example.com", passwordEncoder.encode("daisy")));
        User rosalina = userRepository
            .save(new User(null, "rosalina", "rosalina@example.com", passwordEncoder.encode("rosalina")));
        logger.info("Users inserted: {}", userRepository.count());
      } catch (Exception e) {
    ...
    logger.info("Scores count: {}", scoreRepository.count());
    if (scoreRepository.count() == 0) {
      logger.info("Inserting scores...");
      try {
        User mario = userRepository.findByUsername("mario").orElseThrow();
        User luigi = userRepository.findByUsername("luigi").orElseThrow();
        User peach = userRepository.findByUsername("peach").orElseThrow();
        User bowser = userRepository.findByUsername("bowser").orElseThrow();
        User yoshi = userRepository.findByUsername("yoshi").orElseThrow();
        User toad = userRepository.findByUsername("toad").orElseThrow();
        User wario = userRepository.findByUsername("wario").orElseThrow();
        User waluigi = userRepository.findByUsername("waluigi").orElseThrow();
        User daisy = userRepository.findByUsername("daisy").orElseThrow();
        User rosalina = userRepository.findByUsername("rosalina").orElseThrow();
        ...
        logger.info("Scores inserted: {}", scoreRepository.count());
      } catch (Exception e) {
        logger.error("ERROR inserting scores: ", e);
      }
    } else {
      logger.info("Scores already exist, skipping...");
    }

    logger.info("=== DATA INITIALIZER END ===");
  }
}